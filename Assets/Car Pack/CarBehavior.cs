...
public class CarBehavior : MonoBehaviour
{
    ...
    public float boostDrain = 40f; // units per second
    public float boostRecharge = 20f; // units per second

    [Header("Direction Change Settings")]
    public float directionChangeSnappiness = 25f; // Higher = more instant stop when changing direction
    ...
    void Update()
    {
        if (!tankControl)
        {
            ...
            {
                wheel_L.localRotation = Quaternion.Euler(
                    new Vector3(0, 0, cursteer / maxsteer * 25)
                );
                wheel_R.localRotation = Quaternion.Euler(
                    new Vector3(0, 0, cursteer / maxsteer * 25)
                );
            }
            ...
            if (curdestab > 0)
            {
                sledLBe.rateOverDistance = 5;
                sledRBe.rateOverDistance = 5;
                smokeLBe.rateOverDistance = 2;
                smokeRBe.rateOverDistance = 2;
            }
            else
            {
                sledLBe.rateOverDistance = 0;
                sledRBe.rateOverDistance = 0;
                smokeLBe.rateOverDistance = 0;
                smokeRBe.rateOverDistance = 0;
            }
            sledLFe.rateOverDistance = 0;
            sledRFe.rateOverDistance = 0;
        }
        ...
        if (moveCamera)
            Camera.transform.position = new Vector3(
                ...
                -8 - Mathf.Abs(speed) * 0.15f
            );
        if (OnlyKeyboard_setFalseToTouchControl)
        {
            bool boostKey = Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.Space) || Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt);
            if (boostKey && canStartBoost && Mathf.Approximately(currentBoost, maxBoost))
            {
                boosting = true;
            }
            else if (boostKey && boosting && currentBoost > 0f)
            {
                // Continue boosting until key released or boost runs out
                boosting = true;
            }
            else
            {
        ...
        else
        {
            for (int i = 0; i < Input.touchCount; ++i)
            {
                Vector2 pos = new Vector2(
                    Input.GetTouch(i).position.x / Screen.width,
                    Input.GetTouch(i).position.y / Screen.height
                );
                if (pos.x < 0.2f & pos.y > 0.3f & pos.y < 0.6f)
                {
                    touch_gas = true;
                    touch_stop = false;
                }
                else if (pos.x < 0.2f & pos.y < 0.3f)
                {
                    touch_gas = false;
                    touch_stop = true;
                }
                if (pos.x < 0.8f & pos.x > 0.6f & pos.y < 0.3f)
                {
                    Left();
                    touch_str = false;
        ...
        {
            currentBoost -= boostDrain * Time.deltaTime;
            if (currentBoost < 0f) currentBoost = 0f;
        }
        else
        {
            currentBoost += boostRecharge * Time.deltaTime;
            if (currentBoost > maxBoost) currentBoost = maxBoost;
        }
        // Burst boost logic: can only start boost if bar is full and not currently boosting
        if (!boosting && currentBoost < maxBoost)
        {
            canStartBoost = false;
        }
        if (!boosting && Mathf.Approximately(currentBoost, maxBoost))
        {
            canStartBoost = true;
        }
    ...
    void FixedUpdate()
    {
        if (invesion)
        {
            ...
            if (tankControl)
                Car_Body.AddTorque(-cursteer * Car_Body.mass * 20);
            else
                Car_Body.AddTorque(
                    (cursteer * Car_Body.mass * speed) / (1 + speed * speed * 0.001f)
                );
        }
        else
        {
            ...
            else
            {
                Car_Body.AddTorque(
                    (cursteer * Car_Body.mass * speed) / (1 + speed * speed * 0.001f)
                        + speed * Random.Range(-curdestab, +curdestab) * Car_Body.mass
                );
    ...
    void SmoothStop()
    {
        Car_Body.linearVelocity = Vector2.Lerp(
            Car_Body.linearVelocity,
            Vector2.zero,
            Time.deltaTime * directionChangeSnappiness
        );
        Car_Body.angularVelocity = Mathf.Lerp(Car_Body.angularVelocity, 0f, Time.deltaTime * directionChangeSnappiness);
    }

    void Gas()
    {
        if (lastDirection == -1) // was in reverse
        {
            SmoothStop(); // <- Smooth out velocity instead of snapping
            gas_pedal = 0;
        }
        ...
        if (boosting && currentBoost > 0f) gas_pedal *= boostMultiplier;

        if (curdestab > 0 && speed > 10)
            curdestab -= Time.deltaTime * destabilization * 2;
        else
            curdestab = 0;

        if (power1 > 1 && speed < power1 * 5)
            curdestab = destabilization;
    ...
    void Tormoz()
    {
        if (lastDirection == 1) // was in forward
        {
            SmoothStop(); // <- Smooth out velocity instead of snapping
            gas_pedal = 0;
        }
    ...
    {
        if (cursteer < -1)
            cursteer += Time.deltaTime * 50;

        if (cursteer < maxsteer)
        {
            cursteer += Time.deltaTime * 20;
        }
        else
        {
            cursteer = maxsteer;
            if (!tankControl)
                if (speed > 25)
                {
                    sledLBe.rateOverDistance = 5;
                    sledRBe.rateOverDistance = 5;
                    sledLFe.rateOverDistance = 5;
                    sledRFe.rateOverDistance = 5;
                } //заносы на скорости при полном повороте //drifts at speed at full turn
                else
                {
                    sledLFe.rateOverDistance = 0;
                    sledRFe.rateOverDistance = 0;
